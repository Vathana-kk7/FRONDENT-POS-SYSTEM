...
import {transferRequestData } from '../data/ReportData'

function ImformationTransferRequest() {
    const [cart,setCart]=useState(transferRequestData)
  return (
    <div className='px-5 p-5 rounded-lg shadow-lg border border-gray-200'>
        <h1 className='font-bold text-md text-blue-900 mb-5'>Transfer by Requester</h1>
            ...
            <table className="w-full  text-gray-800">
                <thead className="sticky top-0 z-40 text-sm">
                    <tr className="border-b border-gray-200 bg-gray-100 text-left text-blue-700">
                        <th className="px-5 py-3 font-semibold">Reqested By</th>
                ...
                <tbody className="font-semibold w-full ">
                    {cart.map((item)=>(
                        <tr key={item.id} className="border-b border-gray-100 text-blue-900 hover:bg-gray-100 hover:scale-x-105 transition-all">
                            <td className="px-5 py-2 text-sm">{item.location}</td>
...
export default ImformationTransferRequest